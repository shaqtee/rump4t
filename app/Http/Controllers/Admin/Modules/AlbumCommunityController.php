...
class AlbumCommunityController extends Controller
{
    protected $model;
    protected $helper;
    protected $handler;
    ...
    public function index(Request $request)
    {
        try{
            ...
            $data = [
                'content' => 'Admin/Community/Album/index',
                'title' => 'Data Album',
                'albums' =>  $this->model->with(['albumCommonity'])->whereNotNull('t_community_id')->filter($request)->orderByDesc('id')->paginate($page)->appends($request->all()),
    ...
    public function create()
    {
        try{
            $data = [
                'content' => 'Admin/Community/Album/addEdit',
                'title' => 'Create Album',
    ...
    public function store(Request $request)
    {
        ...
        try{
            ...
            DB::commit();
            return $this->web->successReturn('community.album.photo.semua', 'album_id', $model->id, 'Berhasil Menambah Data, Silahkan Tambahkan Photo');
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    public function update(Request $request, string $id)
    {
        ...
        try{
            ...
            DB::commit();
            return $this->web->update('community.album.semua');
        } catch (\Throwable $e) {
            DB::rollBack();
            if($e instanceof ValidationException){
                return $this->web->error_validation($e);
    ...
    public function destroy(string $id)
    {
        ...
        try{
            ...
            DB::commit();
            return $this->web->destroy('community.album.semua');
        } catch (\Throwable $e) {
            DB::rollBack();
            return $this->handler->handleExceptionWeb($e);
        }
    ...
    public function index_photo(Request $request, $id)
    {
        try{
            $page = $request->size ?? 10;
            $data = [
                'content' => 'Admin/Community/Album/Photo/index',
                'title' => 'Data Photo',
                'photos' =>  $this->photo->with(['photoCommonity'])->where('t_album_id', $id)->filter($request)->orderByDesc('id')->paginate($page)->appends($request->all()),
                'albums' => $this->model->find($id),
                'columns' => $this->photo->columnsWeb(),
                'album_id' => $id,
            ];
            return view('Admin.Layouts.wrapper', $data);
        } catch (\Throwable $e) {
    ...
    public function create_photo($album_id)
    {
        try{
            $data = [
                'content' => 'Admin/Community/Album/Photo/addEdit',
                'title' => 'Create Photo',
