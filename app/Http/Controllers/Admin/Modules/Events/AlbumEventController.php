...
class AlbumEventController extends Controller
{
    ...
    public function index(Request $request)
    {
        try{
            ...
            $data = [
                'content' => 'Admin/Event/Album/index',
                'title' => 'Data Album',
                'albums' =>  $this->model->with(['albumEvent'])->whereNotNull('t_event_id')->filter($request)->orderByDesc('id')->paginate($page),
    ...
    public function create()
    {
        try{
            $data = [
                'content' => 'Admin/Event/Album/addEdit',
                'title' => 'Create Album',
                'albums' => null,
    ...
    public function photo_index(Request $request, $id)
    {
        try{
            $page = $request->size ?? 10;
            $data = [
                'content' => 'Admin/Event/Album/Photo/index',
                'title' => 'Data Photo',
                'photos' =>  $this->modelPhoto->with(['photoEvent'])->where('t_album_id', $id)->filter($request)->orderByDesc('id')->paginate($page)->appends($request->all()),
                'albums' => $this->model->find($id),
                'columns' => $this->modelPhoto->columnsWeb(),
                'album_id' => $id,
            ];
            return view('Admin.Layouts.wrapper', $data);
        } catch (\Throwable $e) {
    ...
    public function photo_create($album_id)
    {
        try{
            $data = [
                'content' => 'Admin/Event/Album/Photo/addEdit',
