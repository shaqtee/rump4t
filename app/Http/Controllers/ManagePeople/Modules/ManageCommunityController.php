...
class ManageCommunityController extends Controller
{
    ...
    public function index(Request $request){
        try{
            $manage = Auth::user();
            $data = [
                'content' => 'ManagePeople/Community/index',
                'title' => 'My Community',
                'community' => $this->model->findOrfail($manage->t_community_id),
            ];
    ...
    public function create()
    {
        try{
            $data = [
                'content' => 'ManagePeople/Community/add',
                'city' => $this->city->get(),
            ];
            return view('ManagePeople.Layouts.wrapper', $data);
        } catch (\Throwable $e) {
    ...
    public function store(Request $request)
    {
        ...
        try{
            ...
            DB::commit();
            return $this->web->store('community.manage.semua');
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        try{
            $data = [
                'content' => 'ManagePeople/Community/edit',
    ...
    public function update(Request $request, $id)
    {
        ...
        try{
            ...
            DB::commit();
            return $this->web->update('community.manage.semua');
        } catch (\Throwable $e) {
            DB::rollBack();
            if($e instanceof ValidationException){
                return $this->web->error_validation($e);
    ...
    public function destroy($id)
    {
        ...
        try{
            ...
            DB::commit();
            return $this->web->destroy('community.manage.semua');
        } catch (\Throwable $e) {
            DB::rollBack();
            return $this->handler->handleExceptionWeb($e);
        }
    ...
    public function user_index(Request $request, $community_id)
    {
        try{
            ...
            $data = [
                'content' => 'ManagePeople/Community/add-manage-people',
                'title' => 'Data User For people Manage',
                'users' => $this->users->where('flag_done_profile', 1)->where('active', 1)->where('flag_community', 'JOINED')->where('t_community_id', $community_id)->with(['community:id,title', 'group:id,name', 'city:id,name'])->whereRelation('membersCommonity', 'flag_manage', 1)->orderByDesc('id')->paginate($page), //->whereRelation('membersCommonity', 'flag_manage', null)
    ...
    public function user_member(Request $request, $community_id)
    {
        try{
            $page = $request->size ?? 10;
            $data = [
                'content' => 'ManagePeople/Community/member',
                'title' => 'Data User Members',
                'users' => $this->users->where('flag_done_profile', 1)->where('active', 1)->where('flag_community', 'JOINED')->where('t_community_id', $community_id)->with(['community:id,title', 'group:id,name', 'city:id,name'])->whereRelation('membersCommonity', 'flag_manage', null)->orderByDesc('id')->paginate($page),
                'groups' => $this->groups->where('active', 1)->get(),
                'community' => $this->model->orderBy('id', 'asc')->get(),
            ];
    ...
    public function leaderboard($community_id)
    {
        try{
            ...
            $data = [
                'content' => 'ManagePeople/Community/leaderboard',
                'title' => 'Leaderboard',
                'leaderboard' => $result,
            ];
