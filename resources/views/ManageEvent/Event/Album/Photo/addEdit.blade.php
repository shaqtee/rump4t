<div class="mt-3">
    <div class="card  box-shadow-0 ">
        ...
        <div class="card-body pt-0">
            @if (isset($photos))
                <form action="{{ route('event.manage-event.album.photo.ubah', ['id' => $photos->id]) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
            @else
                <form action="{{ route('event.manage-event.album.photo.tambah') }}" method="POST" enctype="multipart/form-data">
            @endif
                @csrf
                    <div class="">
