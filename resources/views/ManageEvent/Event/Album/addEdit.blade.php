<div class="mt-3">
    <div class="card  box-shadow-0 ">
        ...
        <div class="card-body pt-0">
            @if (isset($albums))
                <form action="{{ route('event.manage-event.album.ubah', ['id' => $albums->id]) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
            @else
                <form action="{{ route('event.manage-event.album.tambah') }}" method="POST" enctype="multipart/form-data">
            @endif
                @csrf
                    <div class="">
                        <div class="form-group">
                            <label for="t_event_id">Events</label>
                            <input type="text" class="form-control @error('t_event_id') is-invalid @enderror" value="{{ old('t_event_id', isset($albums) ? $albums->albumEvent->title : $event->title) }}" id="t_event_id" placeholder="Event" required autofocus readonly>
                            <input type="hidden" class="form-control @error('t_event_id') is-invalid @enderror" value="{{ old('t_event_id', isset($albums) ? $albums->albumEvent->id : $event->id) }}" name="t_event_id" id="t_event_id" placeholder="Event" required autofocus>
                            {{-- @error('t_event_id')
