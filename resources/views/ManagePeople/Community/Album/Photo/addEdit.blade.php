<div class="mt-3">
    <div class="card  box-shadow-0 ">
        ...
        <div class="card-body pt-0">
            @if (isset($photos))
                <form action="{{ route('community.manage.album.photo.ubah', ['id' => $photos->id]) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
            @else
                <form action="{{ route('community.manage.album.photo.tambah') }}" method="POST" enctype="multipart/form-data">
            @endif
                @csrf
                    <div class="">
                        <div class="form-group">
                            <label for="t_album_id">album</label>
                            @error('t_album_id')
                                <small style="color: red">{{ $message }}</small>
                            @enderror
                            <input type="text" class="form-control @error('t_album_id') is-invalid @enderror"  value="{{ old('t_album_id', isset($photos) ? $photos->photoCommonity->name : $albums->name) }}" id="t_album_id" placeholder="Album" required autofocus readonly>
                            <input type="hidden" class="form-control @error('t_album_id') is-invalid @enderror"  value="{{ old('t_album_id', isset($photos) ? $photos->photoCommonity->id : $albums->id) }}" name="t_album_id" required autofocus>
                            {{-- <select name="t_album_id" id="t_album_id" class="form-control select2" required autofocus>
                                <option label="Choose one" disabled selected>Select album</option>
                                @foreach ($album as $cmnty)
                                    <option value="{{ $cmnty->id }}" 
                                        @if (old('t_album_id', isset($photos) ? $photos->t_album_id : '') == $cmnty->id)
                                            selected
                                        @endif
                                    >
                                        {{ $cmnty->title }}
