<div class="mt-3">
    <div class="card  box-shadow-0 ">
        ...
        <div class="card-body pt-0">
            @if (isset($albums))
                <form action="{{ route('community.manage.album.ubah', ['id' => $albums->id]) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
            @else
                <form action="{{ route('community.manage.album.tambah') }}" method="POST" enctype="multipart/form-data">
            @endif
                @csrf
                    <div class="">
                            ...
                            <select name="t_community_id" id="t_community_id" class="form-control select2" required autofocus>
                                ...
                                @foreach ($community as $cmnty)
                                    <option value="{{ $cmnty->id }}" 
                                        @if (old('t_community_id', isset($albums) ? $albums->t_community_id : '') == $cmnty->id)
                                            selected
                                        @endif
                                    >
                                        {{ $cmnty->title }}
                        ...
                        <div class="form-group">
                            ...
                            <div class="row">
                                <div class="col">
                                    <label class="rdiobox"><input value="1" name="active" type="radio" {{ old('active', isset($albums) && $albums->active) == '1' ? 'checked' : '' }} required autofocus> <span>Active</span></label>
                                    <label class="rdiobox"><input value="0" name="active" type="radio" {{ old('active', isset($albums) && $albums->active) == '0' ? 'checked' : '' }} required autofocus> <span>Deactive</span></label>
                                </div>
                            </div>
                        </div>
                    </div>  
