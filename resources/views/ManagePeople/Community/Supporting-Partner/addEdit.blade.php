<div class="mt-3">
    <div class="card  box-shadow-0 ">
        ...
        <div class="card-body pt-0">
            @if (isset($sponsors))
                <form action="{{ route('community.manage.sponsor.ubah', ['id' => $sponsors->id]) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
            @else
                <form action="{{ route('community.manage.sponsor.tambah') }}" method="POST" enctype="multipart/form-data">
            @endif
                @csrf
                    <div class="row">
                        <div class="col">
                            ...
                            <div class="form-group">
                                ...
                                @error('image')
                                    <small style="color: red">{{ $message }}</small>
                                @enderror
                                <img class="image-preview img-thumbnail"width="300" style="display: block;">
                        ...
                        <div class="col">
                            ...
                            <div class="form-group">
                                <label for="link_youtube">Youtube/X</label>
                                @error('link_youtube')
                                    <small style="color: red">{{ $message }}</small>
                                @enderror
                                <input type="text" class="form-control @error('link_youtube') is-invalid @enderror"  value="{{ old('link_youtube', isset($sponsors) ? $sponsors->socialMedia->link_youtube : '') }}" name="link_youtube" id="link_youtube" placeholder="Link" required autofocus>
                            </div>
                        </div>
                    </div>  
